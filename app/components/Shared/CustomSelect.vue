...
const selectTheOption = (item) => {
  ...
  if (props.emitName == null) {
    const dataToSend = {
      selected: typeof item === "object" ? item[keyToShow.value] : item,
      itemsToSelect: originalOptions.value,
      originObj: item
    }
...
<template>
  <div class="custom---select-items-wrapper">
    ...
    <div
      v-if="showOption"
      class="option-holder"
      :class="{ 'box-shadow': props.HaveShadow }"
      :style="{
    ...
    >
      ...
      <div class="options">
        <span class="option" v-for="(item, index) in options" :key="index">
          <span v-if="typeof item === 'object'" @click="selectTheOption(item)">
            {{ item[keyOption] }}</span
          >
    ...
    <Teleport to="body">
      <div
        ...
        v-if="showOption"
        class="closed-div-custom-select"
      ></div>
    </Teleport>
  </div>
</template>
