...
<template>
  <Teleport to="body">
    <div
      :class="props.config.show ? 'shared-show-toaster' : ''"
      class="shared-toaster d-flex items-center justify-content-center"
    >
      <div
        class="message-wrapper px-3 py-2 rounded-1 d-flex gap-2 items-center"
        :class="props.config.theme"
      >
        <span
          v-if="props.config.status === 'error'"
          class="icon-message error-icon"
        >
          <svg
            width="24"
            height="24"
        ...
        <span
          class="text-18"
          :class="props.config.status === 'success' ? 'text-green' : ''"
          >{{ props.config.message }}</span
        >
      </div>
    </div>
  </Teleport>
